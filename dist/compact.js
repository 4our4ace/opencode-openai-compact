...
export function compactUrl(url, config = defaultConfig) {
...
function compactBodyValue(key, value) {
    ...
    if (key === "instructions" && isOpenCodeCompactionDeveloperPrompt(value))
        return undefined;
    return value;
}
...
export function compactBody(body, compactModel = defaultConfig.providers.openai.compactModel, config = defaultConfig) {
    const result = { model: compactModel };
...
export function createCompactHooks(config, store, baseFetch = fetch, options = {}) {
    ...
    async function initWithCompactedInput(providerID, requestInput, init, headers, sessionID) {
    ...
    function wrapFetch(base, providerID, provider) {
        ...
        let baseFetch = typeof previousBase === "function" ? previousBase : base;
        const wrapped = (async (requestInput, init) => {
            ...
            const compacted = await baseFetch(oauthRequest ? chatGPTCodexCompactEndpoint : compactUrl(url, config), {
                ...
                body: JSON.stringify(withStableInstructions(compactBody(body, provider.compactModel, config), stableInstructionsByProvider.get(providerID)?.get(sessionID), config.compactBodyKeys.includes("instructions"))),
            });
            if (!compacted.ok) {
                return compacted;
            }
            ...
            const checkpoint = addCheckpoint(providerID, sessionID, responseID, { messageID: responseMessageID(responseID), createdAt: Date.now() }, items);
        ...
        Object.defineProperty(wrapped, fetchMiddlewareSymbol, {
            value: {
                version: 1,
                base: baseFetch,
                middleware: (next) => wrapFetch(next, providerID, provider),
                attach(middleware) {
    ...
    const hooks = {
        ...
        async config(cfg) {
            ...
            for (const [providerID, compactProvider] of Object.entries(config.providers)) {
                ...
                const protocol = currentFetch[fetchMiddlewareSymbol];
                const isOurWrappedFetch = Boolean(currentFetch[wrappedFetch]);
                if (protocol &&
                    typeof protocol === "object" &&
                    protocol.version === 1 &&
                    typeof protocol.attach === "function" &&
    ...
    if (!multiAuthActive) {
        hooks.auth = {
            ...
            async loader(getAuth) {
                ...
                if (asOpenAIOAuth(auth))
                    return { apiKey: openAIOAuthDummyKey };
                if (apiAuth?.type === "api" && typeof apiAuth.key === "string")
                    return { apiKey: apiAuth.key };
                return {};
            },
        };
    }
